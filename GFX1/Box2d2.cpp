...
namespace Assignments{
	namespace Box2D2{
		...
		Box2D2::Box2D2(std::shared_ptr<Graphics::IGraphics> gfx, std::shared_ptr<System::ISystem> sys)
			:vb(nullptr)
		{
		...
		}//Box2D2::Box2D2(Graphics::IGraphics*gfx, System::ISystem*sys)

		Box2D2::~Box2D2(){
			deleteBox();
		}//Box2D2::~Box2D2()

		void Box2D2::init(void){

		}//void Box2D2::init(void)

		void Box2D2::draw(void){
			...
			gfx->getDevice()->SetStreamSource(0, vb, 0, sizeof(Box2D2Vertex));
		...
		void Box2D2::moveBox(FLOAT x, FLOAT y){
		...
		void Box2D2::moveBox(FLOAT x, FLOAT y, FLOAT width, FLOAT height, DWORD color){
			this->width = width;
			this->height = height;
			this->color = color;
			moveBox(x, y);
		...
		void Box2D2::setVelocity(FLOAT xv, FLOAT yv){
			this->xv = xv;
			this->yv = yv;
		}

		bool Box2D2::newBox(FLOAT x, FLOAT y, FLOAT width, FLOAT height, DWORD color){
			...
			result = gfx->getDevice()->CreateVertexBuffer(
				sizeof(Box2D2Vertex)* 4, D3DUSAGE_WRITEONLY, FVF, D3DPOOL_DEFAULT,
				&vb, nullptr
				);
			if (FAILED(result)){
